...
Complete backup and replication system for **Ubuntu 24.04 + Sway**, targeting a **Dell Precision 5520** (daily driver), **GPD Pocket 3**, **ThinkPad T480s** and **MacBook 10,1**.
...
### What's Backed Up
- `manifests/` — **412 apt packages, 16 snaps, 4 flatpaks, 32 pip packages, 10 third-party apt sources + keys**, regenerated by `backup-dotfiles.sh`
- Sway window manager configuration (safety yellow theme)
- foot terminal colours, `.bashrc` additions (PATH, EDITOR, nvm, the `go()` layout function)
...
### GPD Pocket 3 (or any fresh 24.04 that should become a copy of the daily driver)

```bash
git clone https://github.com/asdfgh0318/adam_laptop ~/ŻYCIE/VIBECODING/adam_laptop
cd ~/ŻYCIE/VIBECODING/adam_laptop
./setup-pocket3.sh            # 12 idempotent phases: sources, 412 apt pkgs, snaps, flatpaks,
                              # toolchains, dotfiles, Pocket 3 hardware, tools, doom, ŻYCIE, groups
./setup-pocket3.sh --list     # or run phases individually: ./setup-pocket3.sh 2 7
```

Then read [docs/MANUAL-CHECKLIST.md](docs/MANUAL-CHECKLIST.md) (secrets, logins, big files) and
[docs/GPD-POCKET3.md](docs/GPD-POCKET3.md) (the portrait panel, touch, audio-by-CPU-variant).
[docs/LAPTOP-vs-POCKET3.pdf](docs/LAPTOP-vs-POCKET3.pdf) is a line-by-line table of what is on the
laptop vs what the script puts on the Pocket 3.

The hardware quirks live in `pocket3/` and are also applied by `install.sh` when it detects a Pocket 3.
...
```
0 20 * * * ~/ŻYCIE/VIBECODING/adam_laptop/scripts/auto-update.sh
```
...
| Component | Fix | File |
|---|---|---|
| 8" 1200×1920 portrait panel | `output DSI-1 transform 90 scale 1.5` | `pocket3/sway-gpd-pocket3.conf` |
| Boot console / plymouth | `fbcon=rotate:1 video=DSI-1:panel_orientation=right_side_up` | `pocket3/grub-gpd-pocket3.cfg` |
| Touch + stylus (GXTP7380) | libinput calibration matrix `0 1 0 -1 0 1` | `pocket3/99-gpd-pocket3-touch.rules` |
| Accelerometer | `ACCEL_MOUNT_MATRIX` | `pocket3/61-gpd-pocket3-sensor-local.hwdb` |
| Audio | `snd-intel-dspcfg dsp_driver=1` — **i7-1195G7 only**, auto-detected; N6000 must not get it | `pocket3/alsa-gpd-pocket3.conf` |
| Suspend | `mem_sleep_default=s2idle` | grub cfg |

If the screen comes up upside down: `90` → `270` in the sway line. Details in `docs/GPD-POCKET3.md`.

### Dell Precision 5520 (daily driver)

Generic Intel path (`03-drivers-generic.sh`). Everything in `manifests/` and `dotfiles/` was captured from this machine.
...
```
adam_laptop/
├── install.sh                 # Master installer (hardware-detecting menu)
├── setup-pocket3.sh           # Full replication: manifests + dotfiles + hardware, 12 phases
├── update.sh                  # Sync/backup tool
├── session-logger.sh          # Claude Code hook
├── package-list.txt           # Full dpkg snapshot (reference only)
├── manifests/                 # What setup-pocket3.sh actually installs
│   ├── apt-packages.txt       # apt-mark showmanual minus apt-exclude.txt
│   ├── snaps.txt, snaps-classic.txt, flatpaks.txt, pip-user.txt
│   ├── apt-sources/           # third-party .sources/.list files
│   └── keyrings/              # their signing keys
├── pocket3/                   # GPD Pocket 3 hardware quirk files
...
│   ├── 02-drivers-t480s.sh    # ThinkPad T480s drivers
│   ├── 02-drivers-pocket3.sh  # GPD Pocket 3 (wraps setup-pocket3.sh phase 7)
...
│   ├── bashrc-additions.sh    # PATH, EDITOR, nvm, go()
│   ├── claude/                # Claude Code settings
│   └── etc/modprobe.d/        # Driver configs
│
└── docs/
    ├── GPD-POCKET3.md         # Pocket 3 guide
    ├── MANUAL-CHECKLIST.md    # secrets / logins / big data — the unscriptable part
    ├── LAPTOP-vs-POCKET3.pdf  # comparison table
